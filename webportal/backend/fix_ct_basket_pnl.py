"""
fix_ct_basket_pnl.py

Derives all buy/sell events for Consumer_Trends basket from Excel rebalance history,
fetches OHLC prices (Yahoo Finance -> Google Finance -> Screener.in),
computes FIFO P&L, and updates:
  - buy_price_data.json   (Consumer_Trends section)
  - gains_statement.json  (Consumer_Trends section replaced)
  - portfolios.json       (Consumer_Trends_sold rebuilt)

Run: python fix_ct_basket_pnl.py
"""
...
EXCEL_PATH  = Path(r"C:\Users\PC\OneDrive - Niveshaay Investment Advisors (1)\Nukul Madaan NIA\Niveshaay Office Work\Rebalance Historical Data\Historical Index Values_22.05.2026\Consumer Trends Index Value.xlsx")
BASKET      = "Consumer_Trends"
...
async def main():
    ...
    # Backup
    shutil.copy(bp_path,   BASE / "buy_price_data_backup_before_ct_fix.json")
    shutil.copy(gs_path,   BASE / "gains_statement_backup_before_ct_fix.json")
    shutil.copy(port_path, BASE / "portfolios_backup_before_ct_fix.json")
    print("Backups created.")

    # Build name→NSE map
    ct_all = portfolios.get(BASKET, []) + portfolios.get(f"{BASKET}_sold", [])
    ...
    nse_to_name: dict[str, str] = {s["nseCode"]: s.get("securityName", s["nseCode"]) for s in ct_all}
    ...
    for nse, series_list in stock_series.items():
        ...
        if len(series_list) == 1:
            ...
            # 3+ series (e.g. LIQUIDCASE) — store all prev series combined; last as current
    ...
    for nse, series_list in stock_series.items():
        ...
        if len(series_list) == 1:
            ...
            # 3+ series: compute each independently, combine all into prevSeriesGains
    ...
    gains_stmt[BASKET] = ct_gains

    # ── Rebuild Consumer_Trends_sold ──────────────────────────────────────────
    active_nse = {nse for nse, sl in stock_series.items() if sl[-1]["is_active"]}
    sold: list[dict] = []
    for nse, data in ct_gains.items():
    ...
    print(f"\n{'='*60}")
    print(f"  Consumer_Trends P&L rebuilt: {len(ct_gains)} stocks with gains records")
    print(f"  Consumer_Trends_sold rebuilt: {len(sold)} records")
